...
class Maze(object):
    ...
    def destroy_deadends(self) -> None:
        ...
        for y, maze_row in enumerate(self.maze_data):
            for x, maze_tile in enumerate(maze_row):
                if maze_tile == BlockType.GROUND and self.num_exits((x, y)) == 1 and not (x, y) == self.entrance:
                    deadends.add((x, y))
    ...
    def get_keydists(self):
        keydists = dict()
        desired_locs = set(self.keys.keys()).union([self.entrance])
        for loc, name in self.keys.items():
            explored = set(loc)
            to_explore = [(loc, 0)]
            while to_explore:
                loc, dist = to_explore.pop(0)
                if loc in desired_locs:
                    neighbor_name = self.keys[loc] if loc in self.keys else '0'
                    keydists[(name, neighbor_name)] = keydists[(neighbor_name, name)] = dist

                for d in Direction:
                    neighbor = add_dir(loc, d)
                    if neighbor not in explored and self.at(neighbor) != BlockType.WALL:
                        explored.add(neighbor)
                        to_explore.append((neighbor, dist+1))
        return keydists
